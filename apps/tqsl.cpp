...
DupesDialog::DupesDialog(wxWindow *parent, int qso_count, int dupes) :
	...
	if (qso_count == dupes) {
		...
		wxT("while allowing duplicate QSOs."),
			dupes, newq, 
			(newq == 1) ? wxT("which is") : wxT("s which are"));
	}
...
public:
	...
	bool ConvertLogToString(tQSL_Location loc, wxString& infile, wxString& output, int& n, tQSL_Converter& converter, bool suppresdate=false, const char* password=NULL);
	int UploadLogFile(tQSL_Location loc, wxString& infile, bool compress=false, bool suppressdate=false, const char* password=NULL);
...
bool MyFrame::ConvertLogToString(tQSL_Location loc, wxString& infile, wxString& output, int& n, tQSL_Converter& conv, bool suppressdate, const char* password) {
...
restart:
	...
	int processed = 0;
   	try {
		...
		wxString ident = wxString::Format(wxT("%s Lib: V%d.%d Config: V%d.%d AllowDupes: %s"), wxString(iam, wxConvLocal).c_str(),
			major, minor, config_major, config_minor,
			allow_dupes ? wxT("true") : wxT("false"));
...
MyFrame::ConvertLogFile(tQSL_Location loc, wxString& infile, wxString& outfile,
	...
	bool cancelled=this->ConvertLogToString(loc, infile, output, numrecs, conv, suppressdate, password);
...
int MyFrame::UploadLogFile(tQSL_Location loc, wxString& infile, bool compressed, bool suppressdate, const char* password) {
	int numrecs=0;
	wxString signedOutput;
	tQSL_Converter conv=0;
	bool cancelled=this->ConvertLogToString(loc, infile, signedOutput, numrecs, conv, suppressdate, password);
	...
	if (cancelled || numrecs==0) {
		...
		if (retval==0) { //success
			...
			wxLogMessage(wxT("Couldn't upload the file: CURL returned %s"), wxString::FromAscii(errorbuf).c_str());
			retval=4;
		}
...
void MyFrame::DoCheckForUpdates(bool silent) {
	...
	wxString updateURL=config->Read(wxT("UpdateURL"), wxT("http://www.rmead.com/tqslupdate/tqslupdate.txt"));
	...
	if (curl_easy_perform(req) == CURLE_OK) {
		...
		if (header.StartsWith(wxT("TQSLVERSION;"), &onlinever)) {
			if (onlinever.Cmp(wxT(VERSION))>0) {
				...
				if (!silent)
					wxMessageBox(wxString::Format(wxT("Version %s is the newest available"), wxT(VERSION)), wxT("No Updates"), wxOK|wxICON_INFORMATION, this);
			}
		} else {
			if(!silent)
...
MyFrame::ImportQSODataFile(wxCommandEvent& event) {
	...
	try {
		...
		if (wxMessageBox(wxString::Format(wxT("The file (%s) will be signed using:\n"
                                          wxT("Station Location: %s\nCall sign: %s\nDXCC: %s\nIs this correct?")), infile.c_str(), wxString::FromAscii(loc_name).c_str(),
...
QSLApp::OnInit() {
	...
	static const wxCmdLineEntryDesc cmdLineDesc[] = {
		...
		{ wxCMD_LINE_SWITCH, wxT("p"), wxT("password"),	wxT("Password for the signing key") },
	...
	if (upload) {
		try {
			int val=frame->UploadLogFile(loc, infile, true, suppressdate, password);
			exit(val);
		} catch (TQSLException& x) {
			wxString s;
			if (!infile.empty())
